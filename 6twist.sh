...
function dependenciesinstall(){
    if [ "$packagemanagertype" = "a" ]; then
        ...
        apt-get -yq install wget gawk grep curl sed git gcc swig gettext   make  perl cpio xmlto asciidoc cron fail2ban net-tools dnsutils rng-tools libc-ares-dev libev-dev openssl libssl-dev zlib1g-dev libpcre3-dev libevent-dev build-essential python-dev python-pip python-setuptools python-m2crypto
        ...
        yum -y install wget gawk grep curl sed git gcc swig gettext-devel   make  pcre-devel perl-devel cpio xmlto asciidoc vixie-cron crontabs fail2ban net-tools bind-utils rng-tools expat-devel openssl-devel zlib-devel libev-devel c-ares-devel python-devel python-setuptools python-pip
        [ "$?" != "0" ] && dependenciesinstallerr "Install"
        [ "$FWS" = "enable" ] && { yum -y install ${webservername} || dependenciesinstallerr "Apache"; }
    else
        pacman -Syyu --noconfirm || dependenciesinstallerr "Update and Upgrade"
        pacman -Syu --noconfirm wget gawk grep curl sed git gcc swig gettext   make  pcre perl cpio xmlto asciidoc cronie fail2ban net-tools rng-tools openssl zlib c-ares libev python-devel python-setuptools python-pip python2-m2crypto
        [ "$?" != "0" ] && dependenciesinstallerr "Install"
...
function sslibevinstall(){
	...
	if [ "$1" = "install" ]; then
        [ -z "$libsodiumver" ] && libsodiumver="$(wget -qO- https://api.github.com/repos/jedisct1/libsodium/releases/14830256 | grep 'tag_name' | cut -d\" -f4)"
